 ...
 */
const moduleSlugs = readdirSync("src/pocs", { withFileTypes: true })
  .filter((entry) => entry.isDirectory() && existsSync(`src/pocs/${entry.name}/db/schema.ts`))
  .map((entry) => entry.name);

const pgSchemaOf = (slug: string) => slug.replaceAll("-", "_");

const only = process.env.DRIZZLE_MODULE || undefined;
if (only && only !== "platform" && !moduleSlugs.includes(only)) {
...
function scope() {
  ...
  return {
    schema: ["./src/core/db/schema.ts", ...moduleSlugs.map((s) => `./src/pocs/${s}/db/schema.ts`)],
    schemaFilter: ["public", ...moduleSlugs.map(pgSchemaOf)],
  };
}
