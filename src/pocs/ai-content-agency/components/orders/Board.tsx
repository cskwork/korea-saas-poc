...
export function Board({ orders, today, showAllDelivered }: { orders: OrderListItem[]; today: string; showAllDelivered: boolean }) {
  return (
          ...
          <section key={status} id={`tier-${status}`} className={styles.tier} aria-labelledby={`tier-${status}-name`}>
            ...
            {shown.length < items.length ? (
              <p className={styles.tierMore}>
                <Link href="/ai-content-agency/orders?all=1#tier-delivered">납품완료 {items.length}건 모두 보기</Link>
              </p>
            ) : null}
          </section>
        );
